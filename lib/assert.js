...
const isRegExp = obj => obj && obj.constructor.name === 'RegExp';
...
function validateExpectations(assertion, expectations, numArgs) { // eslint-disable-line complexity
	if (typeof expectations === 'function') {
	...
	} else if (typeof expectations === 'string' || isRegExp(expectations)) {
		...
		if (hasOwnProperty(expectations, 'message') && typeof expectations.message !== 'string' && !(isRegExp(expectations.message))) {
			throw new AssertionError({
				assertion,
...
function assertExpectations({assertion, actual, expectations, message, prefix, stack}) {
	...
	if (isRegExp(expectations.message) && !expectations.message.test(actual.message)) {
		throw new AssertionError({
			assertion,
			message,
			stack,
