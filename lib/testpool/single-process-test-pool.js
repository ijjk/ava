...
class SingleProcessTestPool extends TestPool {
	...
	run() {
		...
		return Bluebird.map(this.files, file => {
			...
			// TODO: this is a costly call, figure out a way to optimize.
			// Maybe research further into why `import-fresh` wasn't working
			// as a replacement for require
			Object.keys(require.cache).forEach(mod => delete require.cache[mod]);
